...
- Never tell the agent that its result was truncated. The cut is in YOUR copy only: the agent received the whole result and has already read it. Asking it for an "untruncated source", to "re-check the full document", or saying it "has no evidence" because your copy is cut is wrong every time. If the part you would need is missing from your copy, you have nothing to say about it — stay silent on that point.
