...
<no_intervention/>`,
  ...
  `<memory_save_knowledge id="k2">User has not yet confirmed the refund destination (original card vs gift card).</memory_save_knowledge>
<context_for_action>You are about to run return_delivered_order_items on #W2378156, but the user has not confirmed the refund destination. List item 1151293680 ($272.51) and ask whether the refund goes to credit_card_9513926 or a gift card, and wait for an explicit yes.</context_for_action>`,
]
...
/** Drive one pre-step exactly the way dsh-agent-loop does: default decision = [...claimed, context]. */
async function step({ claimed, turn, at }) {
  ...
  log.push(...claimed) // the loop appends the claimed messages at step/start
  return { out, base, context }
}

rule('STEP 1 — the memory model is consulted and stays silent')
...
// The executor now tries a write; the plugin observes it (never denies) for <about_to_act>.
await preExecute(
...
rule('STEP 2 — the memory model interrupts, and one reminder is spliced')
const second = await step({ claimed: [user('go ahead')], turn: 3, at: 1 })
line('\n· <about_to_act> the memory model saw:')
line(`    ${/<about_to_act>\n([\s\S]*?)\n<\/about_to_act>/.exec(textOf(lastRequest.messages[0]))?.[1] ?? '(none)'}`)
...
line(`\n· the reminder sits at index ${second.out.messages.indexOf(reminder)}: after the claimed user message, before the runtime context.`)
