...
| 3 | Sees the task, the last k=8 messages, and its own bank. | The same three (`<task>`, `<transcript window="8">`, `<memory_bank>`, JSON-framed, tool results middle-truncated to 4000 chars — enough that a retail tool result arrives whole) **plus** the domain `<policy>` in its system prompt and three episode-cumulative sections the window cannot hold: `<key_tool_calls>`, `<executed_writes>`, `<recent_writes>`. | **More** | Transcript text can contain anything; JSON is a delimiter it cannot break out of, and truncation bounds the cost. The additions are answers to measured failures of the k=8 window alone — see the last rows of the additions table below. |
